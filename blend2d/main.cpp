   ...
   public:
	...
	void onMouseEvent(QMouseEvent* event) {
		...
		if (event->type() == QEvent::MouseMove) {
			if (_grabbedVertex == SIZE_MAX) {
				_path.getClosestVertex(BLPoint(double(event->x()), double(event->y())), 5,
									   &_closestVertex);
				;
				_canvas.updateCanvas();
			} else {
	...
	Q_SLOT void onSetRandom() {
		...
		if (_prng.nextDouble() < 0.5) _path.close();
	...
	void renderPathPoints(BLContext& ctx, const BLPath& path, size_t highlight,
						  BLRgba32 normalColor, BLRgba32 highlightColor) noexcept {
		size_t count = path.size();
		const BLPoint* vtx = path.vertexData();

		for (size_t i = 0; i < count; i++) {
			if (!std::isfinite(vtx[i].x)) continue;
			if (i == highlight)
